...
namespace CC::AddAchievement
{
	namespace detail
	{
		inline constexpr auto LONG_NAME = "AddAchievement"sv;
	...
	inline void Install()
	{
		...
		const auto it = std::find_if(
			...
			functions.end(),
			[&](auto&& a_elem) {
				return _stricmp(a_elem.functionName, "AddAchievement") == 0;
			});
		if (it != functions.end()) {
